...
module DockerAutoBuild
  class BuildJob
    ...
    def clone_repository
      git_clone_command = Command.run(command: ['git', 'clone', "--branch=#{@branch}", '--depth=1', @repository, @random_directory_name])
      raise "Cannot clone repositroy: #{@repository}, branch: #{@branch}"
    end
    ...
    def get_project_name_from_repo_url
      matchdata = @repository_url.match(/.*github.com\/(.*)\/(.*).git/)
      raise("Bad github repository: #{@repository_url}") if matchdata.nil?
      org, project_name = matchdata.captures
      project_name
    end
